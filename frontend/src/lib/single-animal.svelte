  ...
  {:then data}
    <div class="photos">
      <div class="slider-container">
        <Carousel let:loaded>
          {#each data as animal, index}
            {#each [animal.pictures, animal.pictures2, animal.pictures3] as picture}
              {#if picture}
      ...
      <button on:click={rtn} class="return_button">
    ...
    <div class="presentation-single">
      <ul class="icons">
        ...
        {#if data[0].ok_dog == 1}
          <li>
            <img
              class="icons_img"
              src="./src/assets/icons/chien.png"
              alt="chien"
            />
          </li>
        {/if}
      ...
      <div class="button">
        <a class="btn" aria-label="page de contact" href="/contact" use:link
          >Je veux l'adopter !</a
        >
      </div>
    </div>
